...
###################################################################
def Match_Arduinos ():
    logger = logging.get_logger("Match_Arduinos()")
    ...
    for p in ports:                                                 # For every available port
        logger.debug("%s | %s", str(p), p.hwid)                     # Log the Name and the port location
        for a in Arduino_List:                                      # For every Arduino Specified in the main code
            if a[1] in p.hwid:                                      # Check of the Serial Number matches the Serial Number of the Port
                ...
                logger.info("Port %s is %s ", p.device, a[0])
    
    logger.info("------Matches Made------")                               # List valid Matches
    for a in Arduino_List:
        if a[2] != "":
            logger.info("%s | %s", a[0], a[2])
            
    logger.info("------Matches NOT Made------")                         # List Matches not made
    for a in Arduino_List:
        if a[2] == "":
            logger.info("%s was not found", a[0])
            
    logger.info("-----------------------------")                          # Terminal output formatting
###################################################################         
...
def Start_Serial(port):                                             # Start-Up a serial interface
    logger = logging.get_logger("Start_Serial()")
...
class Create_Serial(serial.Serial):                                 # Create Serial Object with custom Methods (Custom)
    ...
    def get_data(self):
        logger = logging.get_logger("get_data()    ")
        try:
            ...
            while(True):
                ...
                if(time.time() >= start_time + wait_max):
                    logger.warning("ERROR: TIMEOUT - Arduino did not reply in time")
                    return("ERROR: TIMEOUT")                            # Returns TIMEOUT if arduino didn't reply in time
        except:
            logger.warning("ERROR: Serial Communication Error")
...
class start_serial_connections():
    ...
    def reconnect(self, arduino_name):
        ...
        logger.debug("Reconnecting %s", arduino_name)
        if not "ERROR" in eval(f"self.{arduino_name}.test_command()"):
            logger.debug("Port for %s is functioning properly, no reset needed", arduino_name)
            return (True)
        ...
        try:
                ...
                for a in Arduino_List:
                    if a[0] == arduino_name:
                        if a[1] in p.hwid:
                            if a[2] != p.device:
                                a[2] = p.device
                                logger.debug("Port location for %s has been changed to %s", arduino_name, a[2])
            ...
            for a in Arduino_List:
                if Arduino_List[i][2] != "" and Arduino_List[i][0] == arduino_name:
                    ...
                    return (True)
                i += 1
            logger.debug("Re-connect Failed")
            return (False)
